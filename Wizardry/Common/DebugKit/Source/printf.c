...
static u64 local_udiv64(u64 a, u64 b)
{
	u64 ret = 0;
	u64 tmp_a = a;
	u64 tmp_b = b;
...
static u64 local_umod64(u64 a, u64 b)
{
	u64 ret = 0;
	u64 tmp_a = a;
	u64 tmp_b = b;
...
static int vsnprintf_puts(const char *str, size_t len, void *_arg)
{
...
int vsnprintf(char *str, size_t len, const char *fmt, va_list ap)
{
	...
	wlen = kprintf_engine(vsnprintf_puts, &arg, fmt, ap);
	if (len == 0)
		;
	else if (arg.pos >= len)
		str[len - 1] = '\0';
...
static int fprintf_puts(const char *str, size_t len, void *_arg)
{
	FILE *fp = _arg;
...
{
	return kprintf_engine(fprintf_puts, fp, fmt, ap);
}
